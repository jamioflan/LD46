...
public class OnScreenObjectiveMarker : MonoBehaviour
{
	public static OnScreenObjectiveMarker the;
    ...
    void Update()
    {
		if (target != Vector3.zero)
		{
			...
			Vector3 objectivePosition = target;
			objectivePosition.y += 0.5f;
			...
			if (behind)
			{
				if(Vector3.Dot(ray, -viewer.transform.right) < Vector3.Dot(ray, viewer.transform.right))
				{
